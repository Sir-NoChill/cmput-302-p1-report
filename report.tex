...
The current colour scheme (Purple (\verb|#07012F|), Blue (\verb|#0191FD|) and Red (\verb|#FC5C00|)) is jarring to the eyes. The literature establishes that red and purple are particularly hard for users to look at for extended periods of time \cite{jonesHumancomputerInteractionDesign1989}. Furthermore, there are instances where red is used as the colour for the program's desired state when presented with multiple choices. This may have confusing implications towards the user as red is often associated with being a 'negation' or 'emergency' choice when that is not the intention. 
